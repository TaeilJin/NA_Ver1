...
class NFDiag(nn.Module):
    def __init__(self, nx, ny, nk, nh_mlp,nh_rnn,rnn_type,x_birnn,fix_first):
        super(NFDiag, self).__init__()
        self.nx = nx # input dimension
        self.ny = ny # 
        self.nk = nk
        self.nh = nh_mlp 
        self.nh_rnn = nh_rnn 
        self.rnn_type = rnn_type 
        self.x_birnn = x_birnn 
        self.fix_first = fix_first 
        self.nac = nac = nk - 1 if fix_first else nk
        self.x_rnn = RNN(nx, nh_rnn, bi_dir=x_birnn, cell_type=rnn_type)
        self.mlp = MLP(nh_rnn, nh_mlp)
        self.head_A = nn.Linear(nh_mlp[-1], ny * nac)
        self.head_b = nn.Linear(nh_mlp[-1], ny * nac)

    def encode_x(self, x):
        if self.x_birnn:
            h_x = self.x_rnn(x).mean(dim=0)
        else:
            h_x = self.x_rnn(x)[:,-2:-1,:]
        return h_x
