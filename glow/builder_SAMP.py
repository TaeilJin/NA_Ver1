...
from .models_SAMP import MotionNet,FootContactNet
from .utils import load_SAMP, save, get_proper_device
import numpy as np
...
def build_SAMP(x_channels, cond_channels, hparams, is_training):
    ...
    kwargs = {"rng": 3, "z_dim": 64, "num_experts": 12}
    rng = np.random.RandomState(23456)
    kwargs['rng'] = rng
    if hparams.Train.model == "foot_predictor":
        graph = FootContactNet(x_channels + cond_channels,**kwargs)
    ...
    if graph is not None:
        ...
        if pre_trained is not None:
            loaded_step = load_SAMP(os.path.basename(pre_trained),
