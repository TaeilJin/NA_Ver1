...
class conditionEncoder_LSTM(nn.Module):
    def __init__(self,in_channels, cond_channels, hidden_channels, out_channels, num_layers,pose_channels):
    ...
    def forward(self, input):
        ...
        if(self.pose_channels > 0):
            ...
            input = torch.cat((input[:,:,:-(self.env_channels)],env),dim=-1)
        output = self.layer_lstm(input)

        return output

def f(in_channels, out_channels, hidden_channels, cond_channels, network_model, num_layers,pose_channels=66):
    ...
    if network_model =="LSTM_ENV":
        return conditionEncoder_LSTM(in_channels, cond_channels, hidden_channels, out_channels, num_layers,pose_channels)
...
class FlowStep(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, cond_channels,
        ...
        if flow_coupling == "additive":
            self.f = f(in_channels // 2, in_channels-in_channels // 2, hidden_channels, cond_channels, network_model, num_layers,pose_channels)
        elif flow_coupling == "affine":
            print("affine: in_channels = " + str(in_channels))
            self.f = f(in_channels // 2, 2*(in_channels-in_channels // 2), hidden_channels, cond_channels, network_model, num_layers,pose_channels)
...
class SelectLayerEE(nn.Module):
    def __init__(self):
        super().__init__()
        self.upper_idx = [(5)*3+ 0,(5)*3+ 1,(5)*3+ 2,
        (9)*3+ 0,(9)*3+ 1,(9)*3+ 2,
...
class FlowNet(nn.Module):
    def __init__(self, x_channels, hidden_channels, cond_channels, K,
        ...
        # flow end end-effectors
        self.select_layer_ee = SelectLayerEE()
        self.layers_ee = nn.ModuleList()
        for _ in range(K):
            self.layers_ee.append(
                FlowStep(in_channels=self.select_layer_ee.getdim(),
                         hidden_channels=hidden_channels,
                         cond_channels=N + 10 + num_foots,
            ...
            self.output_shapes.append(
                [-1, self.select_layer_ee.getdim(), 1])
        ...
        for _ in range(K):
            self.layers_u.append(
                FlowStep(in_channels=self.select_layer_u.getdim(),
                         hidden_channels=hidden_channels,
                         cond_channels=N +x_channels + 10 + num_foots,
                         actnorm_scale=actnorm_scale,
                         flow_permutation=flow_permutation,
                         flow_coupling=flow_coupling,
                         network_model=network_model,
    ...
    def forward(self, z, cond, ee_cond=None, label = None, foot=None, logdet=0., reverse=False, eps_std=None):
        if not reverse:
            # train x->z
            # pose mask condition vector 
            #masked_init = torch.zeros(masked_u.shape).to(z.device)
            
            # select end effector first (fixed hierarchy)
            z_ee, masked_ee = self.select_layer_ee(z)
            # # # flow steps input : x_ee 
            z_ee_cond = torch.cat((foot,label,cond),dim =1)
            for layer in self.layers_ee:
                z_ee, z_ee_cond, logdet = layer(z_ee, z_ee_cond, logdet, reverse=False)
            
            
            # select upper body first, (fixed hierarcy)
            z_u,masked_u = self.select_layer_u(z) 
            # flow steps input : x_upper, pose masked ee condition 
            z_u_cond = torch.cat((foot,label,cond,masked_ee),dim =1)
            for layer in self.layers_u:
            ...
            masked_u = masked_u + masked_ee
            # flow steps input : x_lower, pose mask condition of upper
            z_l_cond = torch.cat((foot,label,cond,masked_u),dim =1)
            for layer in self.layers_l:
                z_l, z_l_cond, logdet = layer(z_l, z_l_cond, logdet, reverse=False)
            
            # input new  z & logdet
            z = self.select_layer_ee.addElement(z,z_ee)
            ...
            z_ee_cond = torch.cat((foot,label,cond),dim =1)
            # flow steps
            for i,layer in enumerate(reversed(self.layers_ee)):
                z_ee, z_ee_cond, logdet = layer(z_ee, z_ee_cond, logdet=0, reverse=True)
            # !! z_u after reversing all flow steps, is a sampled x_u
            masked_ee = self.select_layer_ee.addElement(masked_ee,z_ee)
            
            # input End-Effector positions (desired)
            if (ee_cond is not None):
                masked_ee = self.select_layer_u.addEndEffectorElement(masked_ee,ee_cond) # upper body endeffector positions
            ...
            # create x space 
            if (ee_cond is not None):
                z = self.select_layer_ee.addElement(z,ee_cond)
            else:
                z = self.select_layer_ee.addElement(z,z_ee)
            z = self.select_layer_u.addElement(z,z_u)
            z = self.select_layer_l.addElement(z,z_l)
...
class HierGlow_label_ENV_FOOT_3PART(nn.Module):
    ...
    def __init__(self, x_channels, cond_channels, hparams):
        ...
        self.flow = FlowNet(x_channels=x_channels,
                            ...
                            num_foots=hparams.Glow.num_foots,
    ...
    def forward(self, x=None, cond=None, ee_cond=None, label= None, foot=None, z=None, 
                eps_std=None, reverse=False):
        if not reverse:
            return self.normal_flow(x, cond, ee_cond, label, foot)
        else:
            return self.reverse_flow(z, cond, ee_cond, label,foot,eps_std)

    def normal_flow(self, x, cond, ee_cond, label, foot):
        ...
        # encode
        z, objective = self.flow(x, cond, ee_cond,label=label, foot=foot, logdet=logdet, reverse=False)
    ...
    def reverse_flow(self, z, cond, ee_cond, label,foot,eps_std):
        with torch.no_grad():
            ...
            x = self.flow(z, cond, ee_cond, label = label, foot=foot, eps_std=eps_std, reverse=True)
        return x
