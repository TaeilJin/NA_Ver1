...
class Trainer_Label_Foot(object):
    ...
    def train(self):
        ...
        for epoch in range(self.n_epoches):
            ...
            for i_batch, batch in enumerate(progress):
                ...
                # forward phase
                z, nll = self.graph(x=x, cond=cond, ee_cond = ee_cond, label = prob_label, foot=foot)
                ...
                with torch.no_grad():
                    # sample with 
                    if hasattr(self.graph, "module"):
                        sampled_z_all = self.graph.module.loss_fn_GMM.sample_all(nBatch_cond*nTimesteps_cond)
                    else:
                        sampled_z_all = self.graph.loss_fn_GMM.sample_all(nBatch_cond*nTimesteps_cond)

                    sampled_z_all = sampled_z_all.unsqueeze(0)
                    sampled_z_all = sampled_z_all.reshape(self.graph.means.shape[0],nBatch_cond*nTimesteps_cond,-1)
                    sampled_z_all = sampled_z_all.permute(1,0,2).to(self.data_device)
                    # sample from Moglow
                    prob_zll = prob.unsqueeze(1).clone().detach()
                    sampled_z_label = torch.bmm(prob_zll,sampled_z_all).clone().detach()

                    #
                    sampled_z_label = sampled_z_label.reshape(nBatch_cond,-1,66).permute(0,2,1)

                # hat to inverse
                x_hat = self.graph(z= sampled_z_label, cond=cond, ee_cond=ee_cond, label=prob_label, foot = foot, reverse=True)

                x_hat_pose = exp_utils.unNormalize_motion(x_hat.permute(0,2,1),self.mean,self.scale).clone().detach()
                
                x_foot_vel = x_hat_pose[:,1:,:] - x_hat_pose[:,:-1,:]
                
                joints_vel_fr = torch.abs(x_foot_vel[:,:,16]) # right foot
                joints_vel_fl = torch.abs(x_foot_vel[:,:,20]) # left foot
                joints_vel_tr = torch.abs(x_foot_vel[:,:,17]) # right toe
                joints_vel_tl = torch.abs(x_foot_vel[:,:,21]) # left toe
                ...
                if self.global_step % self.validation_log_gaps == 0:
                    ...
                    for ii, val_batch in enumerate(self.val_data_loader):
                        ...
                        with torch.no_grad():
                            ...
                            prob_label = prob.reshape(nBatch_cond,nTimesteps_cond,-1).permute(0,2,1).clone().detach()
                            z_val, nll_val = self.graph(x=x_val, cond=cond_val, ee_cond=ee_cond_val, label=prob_label, foot = foot_val)
                            ...
                            # hat to inverse
                            x_hat = self.graph(z= sampled_z_label, cond=cond_val, ee_cond=ee_cond_val, label=prob_label, foot = foot_val, reverse=True)
                            ...
                            foot_loss_val = foot_loss_val + foot_vel_loss

                            nll_val = nll_val + foot_vel_loss 
                ...
                if self.global_step % self.plot_gaps == 0 and self.global_step > 0: 
                    if self.cond_model =="enc_rot":
                        ...
                        self.generator.generate_sample_withRef_cond_foot(self.graph,self.graph_cond, eps_std=1.0, step=self.global_step)
