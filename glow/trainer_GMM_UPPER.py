...
class Trainer_GMM_UPPER(object):
            ...
            for i_batch, batch in enumerate(progress):
                ...
                if self.global_step == 0:
                    self.graph(x[:self.batch_size // len(self.devices), ...],
                               ...
                               upper_cond = x[:self.batch_size // len(self.devices), :42,:]
                               )
                    # re-init LSTM hidden
                    if hasattr(self.graph, "module"):
                        self.graph.module.init_lstm_hidden()
                ...
                # forward phase
                z, nll = self.graph(x=x, cond=cond, ee_cond = ee_cond, upper_cond= x[:,:42,:])

                # multiple gaussian loss
                if hasattr(self.graph, "module"):
                    mult_gauss_ll = self.graph.module.loss_multiple_gaussian(z,prob)
                else:
                    mult_gauss_ll = self.graph.loss_multiple_gaussian(z,prob)
                ...
                if self.global_step % self.validation_log_gaps == 0:
                    ...
                    for ii, val_batch in enumerate(self.val_data_loader):
                        ...
                        with torch.no_grad():
                            ...
                            z_val, nll_val = self.graph(x=x_val, cond=cond_val, ee_cond=ee_cond_val, upper_cond =x_val[:,:42,:])
                            
                            # multiple gaussian loss
                            if hasattr(self.graph, "module"):
                                mult_gauss_ll = self.graph.module.loss_multiple_gaussian(z_val,prob)
                            else:
                                mult_gauss_ll = self.graph.loss_multiple_gaussian(z_val,prob)
                ...
                #self.generator.generate_sample_withRef_cond(self.graph,self.graph_cond,upper_cond=x, eps_std=1.0, step=self.global_step)
