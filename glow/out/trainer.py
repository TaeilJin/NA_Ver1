...
class Trainer(object):
    def __init__(self, graph, optim, lrschedule, loaded_step,
        ...
        self.valid_batch_size = hparams.Train.validation_size
    ...
    def train(self):
        ...
        for epoch in range(self.n_epoches):
            ...
            for i_batch, batch in enumerate(progress):
                ...
                x = batch["x"]          
                cond = batch["descriptor"]
                ee_cond = batch["ee_cond"]
                
                #no ee condition
                #cond = batch["descriptor"][:,:-2640,:]
                #ee_cond = None
                ...
                if self.global_step % self.validation_log_gaps == 0:
                    ...
                    for ii, val_batch in enumerate(self.val_data_loader):
                        ...
                        with torch.no_grad():
                            self.graph.eval()
                            cond = val_batch["descriptor"]
                            x=val_batch["x"]
                            ee_cond=val_batch["ee_cond"]
                            #no ee condition
                            #ee_cond = None
                            #cond = val_batch["descriptor"][:,:-2640,:]
                            # init LSTM hidden
                            if hasattr(self.graph, "module"):
                                self.graph.module.init_lstm_hidden()
                            else:
                            ...
                            z_val, nll_val = self.graph(x=x, cond=cond, ee_cond=ee_cond)
