...
class Glow(nn.Module):
    ...
    def __init__(self, x_channels, cond_channels, hparams):
        ...
        # elif hparams.Glow.distribution == "multiGaussians":
        self.distribution = modules.GaussianDiagPrior()

    def init_lstm_hidden(self):
        self.flow.init_lstm_hidden()

    def forward(self, x=None, cond=None, ee_cond=None, z=None, means=None,var=None,
                eps_std=None, reverse=False):
        if not reverse:
            return self.normal_flow(x = x, cond = cond, ee_cond = ee_cond, means = means,var = var)
        else:
            return self.reverse_flow(z = z, cond = cond, ee_cond = ee_cond, means = means,var = var, eps_std=eps_std)

    def normal_flow(self, x, cond, ee_cond, means, var):
        ...
        # prior
        objective += self.distribution.logp(means,var,z)

        # return
        nll = (-objective) / float(n_timesteps) #np.log(2.) * 
        return z, nll

    def reverse_flow(self, z, cond, ee_cond, means, var, eps_std):
        with torch.no_grad():

            if z is None:
                z = self.distribution.sample(means,var,eps_std, device=cond.device)
