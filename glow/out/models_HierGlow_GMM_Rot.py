...
class FlowNet(nn.Module):
    ...
    def forward(self, z, cond, ee_cond=None, logdet=0., reverse=False, eps_std=None):
        if not reverse:
            ...
            if (ee_cond is not None):
                z_u_cond = torch.cat((cond,hip_input,ee_cond),dim =1)
            else:
                z_u_cond = torch.cat((cond,hip_input),dim =1)
            ...
            if(ee_cond is not None):
                z_l_cond = torch.cat((cond,upper_input,ee_cond),dim =1)
            else:
                z_l_cond = torch.cat((cond,upper_input),dim =1)
...
class HierGlow_GMM_ROT(nn.Module):
