...
class conditionEncoder_LSTM(nn.Module):
    def __init__(self,in_channels, cond_channels, hidden_channels, out_channels, num_layers):
        self.SAB = SAB(1,1,2)
        self.LSTM = modules.LSTM(in_channels + cond_channels, hidden_channels, out_channels, num_layers)
    
    def forward(self, input):
        nBatch, nFeats, nTimesteps = input.shape
        input = input.permute(0,2,1).reshape(-1,nFeats) # (B*T,nFeats)
        input = input.unsqueeze(-1) # (B*T,nFeats,1)
        output = self.SAB(input)
        output = output.permute(0,2,1).reshape(nBatch,nTimesteps,-1).permute(0,2,1) #(B,Feat,Timesteps)
...
class HierGlow_GMM_SA(nn.Module):
