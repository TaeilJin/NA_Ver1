...
class conditionEncoder_LSTM(nn.Module):
    def __init__(self,in_channels, cond_channels, hidden_channels, out_channels, num_layers):
        ...
        self.layer_sab = modules.SAB(1,8,1)
        self.Linear_dim = nn.Linear(8,1)
        ...
        """ ENC ENV """
        # self.Linear_ENV = nn.Linear(2640,256)
        # cond_channels = cond_channels - 2640 + 256 #no Enviornment Encoder
    ...
    def forward(self, input):
        ...
        #end-effector
        ee = input[:,:,-30:]
        nBatch, nTimesteps, nFeats = ee.shape
        ee = ee.reshape(-1,nFeats) # (B*T,nFeats)
        
        # concat env
        cond = ee # for encoding env separately

        # attention
        cond = cond.unsqueeze(-1) # (B*T,nFeats,1)
        cond = self.layer_sab(cond)
        cond = self.Linear_dim(cond)
        cond = cond.permute(0,2,1).reshape(nBatch,nTimesteps,-1)#(B,Feat,Timesteps)
         
        
        input = torch.cat((input[:,:,:-30],cond),dim=-1) # for encoding env separately
        input = self.layer_lstm(input)
        ...
        # env = env.unsqueeze(1).reshape(nBatch,nTimesteps,-1)#(B,Feat,Timesteps) for encoding env separately
        ...
        # input = torch.cat((input[:,:,:-2670],env,cond),dim=-1) # for encoding env separately
        # input = self.layer_lstm(input)
        ...
        # # concat env
        # cond = torch.cat((env,ee),dim=-1) #(BxT,256 + 30)
        ...
        # #input = torch.cat((input[:,:,:-30],cond),dim=-1) # for encoding env separately
        # input = torch.cat((input[:,:,:-2670],cond),dim=-1)
        # input = self.layer_lstm(input)
...
class FlowStep(nn.Module):
    ...
    NetworkModel = ["LSTM", "GRU", "FF", "LSTM_SA"]
    FlowPermutation = {
    ...
    def reverse_flow(self, input, cond, logdet):
       ...
       # 3. actnorm
        ...
        return z, cond, logdet
""" selection layer Hip """
class SelectLayerHip(nn.Module):
    def __init__(self):
        super().__init__()
        self.hipidx = [0,1,2,
        3,4,5]
        
        self.dim = len(self.hipidx)
    ...
    def forward(self, input,reverse=False):
        if not reverse:
            ...
            self.mask[:,self.hipidx,:] = 1
            
            output = input[:,self.hipidx,:]
...
class SelectLayerUpper(nn.Module):
    def __init__(self):
        ...
        self.upper_idx = [element + 3 for element in self.upper_idx]
...
class SelectLayerLower(nn.Module):
    def __init__(self):
        ...
        self.lower_idx = [element + 3 for element in self.lower_idx]
...
class FlowNet(nn.Module):
    def __init__(self, x_channels, hidden_channels, cond_channels, K,
        ...
        # flow Hip selector
        self.select_layer_hip = SelectLayerHip()
        # flow upper body selector
        self.select_layer_u = SelectLayerUpper()
        # flow lower body selector
        self.select_layer_l = SelectLayerLower()
        
        # self.flow_hip_cond_dim = N + 30
        # self.flow_upper_cond_dim = N + x_channels + 30
        # self.flow_lower_cond_dim = N + x_channels + 30
        
        self.flow_hip_cond_dim = N + 30
        self.flow_upper_cond_dim = N + self.select_layer_hip.getdim() + 30
        self.flow_lower_cond_dim = N + self.select_layer_hip.getdim() + self.select_layer_u.getdim() + 30
        
        # flow Hip module
        self.layers_hip = nn.ModuleList()
        for _ in range(K):
            self.layers_hip.append(
                FlowStep(in_channels=self.select_layer_hip.getdim(),
                         hidden_channels=hidden_channels,
                         cond_channels=self.flow_hip_cond_dim,
        ...
        for _ in range(K):
            self.layers_u.append(
                FlowStep(in_channels=self.select_layer_u.getdim(),
                         hidden_channels=hidden_channels,
                         cond_channels=self.flow_upper_cond_dim,
                         actnorm_scale=actnorm_scale,
                         flow_permutation=flow_permutation,
                         flow_coupling=flow_coupling,
                         network_model=network_model,
        ...
        for _ in range(K):
            self.layers_l.append(
                FlowStep(in_channels=self.select_layer_l.getdim(),
                         hidden_channels=hidden_channels,
                         cond_channels=self.flow_lower_cond_dim,
                         actnorm_scale=actnorm_scale,
                         flow_permutation=flow_permutation,
                         flow_coupling=flow_coupling,
                         network_model=network_model,
    ...
    def forward(self, z, cond, ee_cond=None, logdet=0., reverse=False, eps_std=None):
        if not reverse:
            ...
            # select hip first, 
            z_hip, masked_hip = self.select_layer_hip(z)
            
            if(ee_cond is not None):
                z_hip_cond = torch.cat((cond,ee_cond), dim=1)
            else:
                z_hip_cond = cond
            # transfer hip dim
            for layer in self.layers_hip:
            ...
            hip_input = masked_hip[:,:self.select_layer_hip.getdim(),:]
            ...
            if (ee_cond is not None):
                z_u_cond = torch.cat((hip_input,cond,ee_cond),dim =1)
            else:
                z_u_cond = torch.cat((hip_input,cond),dim =1)
            ...
            upper_input = (masked_hip+masked_u)[:,:(self.select_layer_hip.getdim() + self.select_layer_u.getdim()),:]
            ...
            if(ee_cond is not None):
                z_l_cond = torch.cat((upper_input,cond,ee_cond),dim =1)
            else:
                z_l_cond = torch.cat((upper_input,cond),dim =1)
            ...
            # input new  z & logdet
            z = self.select_layer_hip.addElement(z,z_hip)
            ...
            for i,layer in enumerate(reversed(self.layers_hip)):
                z_hip, z_hip_cond, logdet = layer(z_hip, z_hip_cond, logdet, reverse=True)
            # !! z_hip after reversing all flow steps, is a sampled x_hip
            masked_hip = self.select_layer_hip.addElement(masked_hip,z_hip)
...
class HierGlow_GMM_ROT_SA(nn.Module):
