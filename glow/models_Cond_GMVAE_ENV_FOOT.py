...
import numpy as np
class CondGMVAE_FOOT(nn.Module):
    ...
    def forward(self,cond=None,label_prob=None,foot=None, gumbel_temp=1.0, hard_gumbel= 0):
        # data reshape 
        nBatch, nFeatures, nTimesteps = cond.shape
        cond = cond.permute(0,2,1)
        cond = cond.reshape(-1,nFeatures).clone().detach()
        
        _, fFeatures, _ = foot.shape
        foot = foot.permute(0,2,1)
        foot = foot.reshape(-1,fFeatures).clone().detach()
        ...
        # foot
        pi = out_net['prob_cat']
        out_foot = self.foot_estimator(cond[:,:693],cond[:,-2640:],pi)
        loss_foot = self.criterion(self.m_sig(out_foot[:,:2]), foot[:,:2]) + self.criterion(self.m_sig(out_foot[:,2:]), foot[:,2:])
        # 
        loss_total = loss_dic['total'] + self.w_foot * loss_foot
        return loss_total, loss_dic['reconstruction'], loss_dic['gaussian'],loss_dic['categorical'], loss_dic['predicted_labels'], loss_foot
