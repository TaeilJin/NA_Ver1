...
class conditionEncoder_LSTM(nn.Module):
    def __init__(self,in_channels, cond_channels, hidden_channels, out_channels, num_layers):
        ...
        self.root_channels = cond_channels - self.env_channels -10
        self.encoded_env_cond_dim = cond_channels - self.env_channels + self.out_env_channels
        self.layer_lstm = modules.LSTM(in_channels + self.encoded_env_cond_dim, hidden_channels, out_channels, num_layers)
    
    def forward(self, input):
        #environment
        env = input[:,:,(self.in_channel + 10 + self.root_channels) : (self.in_channel + 10 + self.root_channels + self.env_channels)]
        ...
        input = torch.cat((input[:,:,:(self.in_channel + 10 + self.root_channels)],env,input[:,:,(self.in_channel + 10 + self.root_channels + 2640):]),dim=-1)
...
class SelectLayerUpper(nn.Module):
    ...
    def addEndEffectorElement(self,z, ee_cond):
        # 순서가 Head LH, RH, LF, RF 라는 것을 기억하자!
        ee_cond = ee_cond[:,:9,:]
...
class FlowNet(nn.Module):
    ...
    def forward(self, z, cond, ee_cond=None, label = None, logdet=0., reverse=False, eps_std=None):
        if not reverse:
            ...
            z_u_cond = torch.cat((label,cond),dim =1)
            for layer in self.layers_u:
...
class models_NA_SAMP_woEE(nn.Module):
