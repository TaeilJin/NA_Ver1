...
from .models_Cond_GMVAE_ROT_ENV import ROTCondGMVAE
from .utils import load_mcF, save, get_proper_device
from . import modules
...
def build_Imputing(x_channels, cond_channels, hparams, is_training):
    ...
    if hparams.Train.condmodel == "enc":
        graph = modules.LSTM(x_channels, 512, x_channels, 2)
    ...
    if graph is not None:
        ...
        if is_training:
            ...
            pre_trained = hparams.Infer.pre_trained_mcflow
        if pre_trained is not None:
            loaded_step = load_mcF(os.path.basename(pre_trained),
    ...
    return {
        "graph_im": graph,
        "optim_im": optim,
        "lrschedule_im": lrschedule,
        "devices_im": devices,
        "data_device_im": data_gpu if not use_cpu else "cpu",
        "loaded_step_im": loaded_step
    }
