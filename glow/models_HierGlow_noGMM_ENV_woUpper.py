...
class FlowNet(nn.Module):
    ...
    def forward(self, z, cond, ee_cond=None, logdet=0., label=None,reverse=False, eps_std=None, upper_cond =None):
        if not reverse:
            # train x->z
            ...
            z_cond = torch.cat((label,cond,masked_u),dim =1)
            for layer in self.layers_u:
...
class HierGlow_woGMM_ENV_woUpper(nn.Module):
