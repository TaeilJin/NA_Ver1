...
class FlowNet(nn.Module):
    ...
    def forward(self, z, cond, ee_cond=None, logdet=0., reverse=False, eps_std=None, upper_cond =None):
        if not reverse:
            # train x->z
            #masked_init = torch.zeros(z.shape).to(z.device)
            # input End-Effector conditions 
            # flow steps input : x_upper, pose mask condition 
            masked_u = torch.zeros(z.shape).to(z.device)
            if (upper_cond is not None):
                masked_u = self.select_layer_u.addElement(masked_u,upper_cond)

            masked_u = self.select_layer_u.addEndEffectorElement(masked_u,ee_cond)

            z_cond = torch.cat((cond,masked_u),dim =1)
            for layer in self.layers_u:
                z, z_cond, logdet = layer(z, z_cond, logdet, reverse=False)
            ...
            for i,layer in enumerate(reversed(self.layers_u)):
                z, z_cond, logdet = layer(z, z_cond, logdet=0, reverse=True)
...
class Glow_GMM_ENV_UPPER(nn.Module):
    ...
    def forward(self, x=None, cond=None, ee_cond=None, upper_cond=None, z=None, 
                eps_std=None, reverse=False):
        if not reverse:
            return self.normal_flow(x, cond, ee_cond, upper_cond)
        else:
            return self.reverse_flow(z, cond, ee_cond,upper_cond, eps_std)

    def normal_flow(self, x, cond, ee_cond,upper_cond):
        ...
        # encode
        z, objective = self.flow(x, cond, ee_cond, upper_cond = upper_cond, logdet=logdet, reverse=False)
    ...
    def reverse_flow(self, z, cond, ee_cond,upper_cond, eps_std):
        with torch.no_grad():
            ...
            x = self.flow(z, cond, ee_cond, upper_cond=upper_cond, eps_std=eps_std, reverse=True)
        return x
