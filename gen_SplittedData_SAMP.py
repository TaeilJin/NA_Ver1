...
def create_Sequence_CondOnly_OutputData3(seqlen,data,num_ee,num_vel,num_env,num_label,scaler):
        
    # sequence data joint(198) + vel(3) + env(2830) + label(1)
    joint_data = data[:,:,:-(num_ee + num_vel+num_env+num_label)].copy() # joint data
    joint_num = joint_data.shape[-1]
    if num_env is not 0 :
        ee_data = data[:,:,joint_num:(joint_num+num_ee)] # control data
        vel_data = data[:,:,(joint_num+num_ee + num_env):(joint_num+num_ee + num_env +num_vel) ]
        label_data = data[:,:,-num_label:] # label data
    else:
        vel_data = data[:,:,-(num_vel+num_label):] # control data
        label_data = data[:,:,:0] # label data 
    # 
    env_data = data[:,:,(joint_num+num_ee):(joint_num+num_ee+num_env)]
...
def save_split_test_data(scaler,data,data_root,filecnt_start, num_ee, num_vel, num_env, num_label, b_save=False):
    # scaling
    joint_data = data[:,:,:-(num_ee + num_env+num_vel+num_label)] # joint data
    joint_num = joint_data.shape[-1]
    if num_env is not 0 :
        ee_data = data[:,:,joint_num:(joint_num+num_ee)] # control data
        control_data = data[:,:,(joint_num+num_ee+num_env):(joint_num+num_ee +num_env+ num_vel) ]
        label_data = data[:,:,-num_label:] # label data
...
def save_split_trainable_data(scaler, seqlen, data, filecnt_train, num_ee, num_vel, num_envsample, labelinfo=0, b_save=False, data_root=None):
    # scaling
    scaled_data_X = data.copy()
    
    if(b_save):
        # trainable data 
        new_x, autoreg_control,single_control , autoreg_seq_control, autoreg_seq_single_control, new_ee,new_env,new_label = create_Sequence_CondOnly_OutputData3(seqlen,scaled_data_X,num_ee,num_vel,num_envsample,labelinfo,scaler)
        #save
...
scaler_save_root = "/root/home/project/Data/root_position_SAMP"

#loco
data_save_root = "/root/home/project/Data/sliced_root_position/loco"
for i in range(0,10):
    if i == ind_loco:
...
start_valid_num = save_split_trainable_data(scaler,seqlen = 10, data = valid_X, filecnt_train=start_valid_num,num_ee=30, num_vel=3,num_envsample=env_dim,labelinfo=1,b_save=True, data_root = f"{data_output_root}/valid")
start_test_num = save_split_test_data(scaler,test_X,f"{data_output_root}/test",filecnt_start = start_test_num,num_ee=30, num_vel=3,num_env=env_dim,num_label=1,b_save=True)

#mani
data_save_root = "/root/home/project/Data/sliced_root_position/mani"
...
#trans
data_save_root = "/root/home/project/Data/sliced_root_position/trans"
...
for i in range(0,10):
    ...
    start_num = save_split_trainable_data(scaler,seqlen = 10, data = train_X, filecnt_train=start_num,num_ee=30, num_vel=3,num_envsample=env_dim,labelinfo=1,b_save=True, data_root = f"{data_output_root}/train")
#mani
data_save_root = "/root/home/project/Data/sliced_root_position/mani"
for i in range(0,10):
    data_root = f"{data_save_root}/sep{i}"
