...
def create_Sequence_CondOnly_OutputData3(seqlen,data,num_ee,num_vel,num_env,num_label,scaler,f_contact=0):
        
    # sequence data joint(66) + ee(30)+ fcontact(2)+ env(2830) + vel(3) + label(1)
    joint_data = data[:,:,:-(num_ee + f_contact + num_vel+num_env+num_label)].copy() # joint data
    joint_num = joint_data.shape[-1]
    if num_env is not 0 :
        ee_data = data[:,:,joint_num:(joint_num+num_ee)].copy() # control data
        vel_data = data[:,:,(joint_num+num_ee + f_contact + num_env):(joint_num+num_ee + f_contact + num_env +num_vel) ].copy()
    ...
    # 
    env_data = data[:,:,(joint_num+num_ee + f_contact):(joint_num+num_ee+ f_contact +num_env)].copy()
    
    # scaling
    scale_data = np.concatenate((joint_data,ee_data,vel_data),axis=-1)
    scale_data = standardize(scale_data,scaler)
    joint_data = scale_data[...,:joint_num]
    ee_data = scale_data[...,joint_num:-num_vel]
    ...
    return new_x, autoreg_control,single_control , autoreg_seq_control, autoreg_seq_single_control, new_ee,new_env,new_label, new_fcontact


def create_SequentialData(seqlen=False,joint_data=False,ee_data=False,env_data=False,tar_root_data=False,foot_data=False):
   ...
   # current pose (output)
    ...
    new_env = concat_sequence_3(1, env_data[:,seqlen:n_frames,:])
    new_ee = concat_sequence_3(1,ee_data[:,seqlen:n_frames,:])
    new_fcontact = concat_sequence_3(1,foot_data[:,seqlen:n_frames,:])
    
    autoreg_control = concat_sequence_3(seqlen +1, tar_root_data)
    single_control = concat_sequence_3(1, tar_root_data[:,seqlen:n_frames,:])
    
    autoreg_seq = concat_sequence_3(seqlen,joint_data[:,:n_frames-1,:])
    autoreg_seq_control = np.concatenate((autoreg_seq,autoreg_control),axis=-1)
    autoreg_seq_single_control = np.concatenate((autoreg_seq,single_control),axis=-1)
    
    return new_x, autoreg_control,single_control , autoreg_seq_control, autoreg_seq_single_control, new_ee,new_env, new_fcontact
...
def save_split_test_data(scaler,data,data_root,filecnt_start, num_ee, num_vel, num_env, foot_contact = 2 ,num_label = 1, b_save=False):
    # scaling
    joint_data = data[:,:,:-(num_ee + foot_contact+ num_env+num_vel+num_label)] # joint data
    joint_num = joint_data.shape[-1]
    if num_env is not 0 :
        ee_data = data[:,:,joint_num:(joint_num+num_ee)] # control data
        control_data = data[:,:,(joint_num+num_ee+foot_contact +num_env):(joint_num+num_ee + foot_contact + num_env+ num_vel) ]
        f_contact_data = data[:,:,(joint_num + num_ee): (joint_num + num_ee + foot_contact)]
        label_data = data[:,:,-num_label:] # label data
        
    
    env_data = data[:,:,(joint_num+num_ee+foot_contact):(joint_num+num_ee+foot_contact+num_env)]
    
    # scaling
    scale_data = np.concatenate((joint_data,ee_data,control_data),axis=-1)
    scale_data = standardize(scale_data,scaler)
...
def save_split_trainable_data(seqlen, 
                              jointdata, ee_data, tar_root_data,
                              env_data, foot_data,
                              filecnt_train, b_save=False, data_root=None):
    if(b_save):
        new_x, autoreg_control, single_control , autoreg_seq_control, autoreg_seq_single_control, new_ee,new_env, new_foot = create_SequentialData(seqlen,jointdata,ee_data,env_data,tar_root_data,foot_data)
        
        #save
        datafilecnt_train = filecnt_train
        for i in range(0,new_x.shape[0]):
            print("data cnt "+str(datafilecnt_train))
...
def load_npz_data_inFolder(data_root,scaler_root):
        
    bvh_files = get_npzfiles(data_root) # folder 에 있는 모든 npz 들을 얻는다.
    ...
    np.savez_compressed(f'{scaler_root}_{os.path.basename(data_root)}.npz', clips = data_X)
    return data_X

def gen_Scaler_From_Data_inFolder(data_root,data_pose,data_ee,data_tar,scaler_root,b_first=False):
    ...
    scalingdatas = np.concatenate((
            data_pose,
            data_ee,
            data_tar), axis=-1).copy()

    if b_first == True :
        scaler = StandardScaler()
        scaler = gen_scaler(scalingdatas,scaler,b_save =True,data_root = f'{scaler_root}/npz')
...
# ----1
# ----{...}
# ----0.npz (folder 안의 내용들을 concate 함)
# ----1.npz
# ----{...}.npz
# ----npz
# ------mixamo.pkl

Train_data_root = "/root/home/project/mnt/ssd23/TAEIL/SAMP/tar_vel/Train"
Test_data_root = "/root/home/project/mnt/ssd23/TAEIL/SAMP/tar_vel/Test"
b_target_isMat = False;
print("Target root is expressed Matrix? : " + str(b_target_isMat))
print("train data root" + str(Train_data_root))
print("test data root" + str(Test_data_root))

""" Data load & Generate Scaler """
#training_data root
create_dir_save_trainable_data = f'{Train_data_root}/npz'
...
for i in range(0,folder_num):
    #scaling 할 데이터 만들기
    data_X = load_npz_data_inFolder(f"{Train_data_root}/{i}", f"{Train_data_root}/train")
    
    if(b_target_isMat):
        gen_Scaler_From_Data_inFolder(f"{Train_data_root}/{i}",data_X[...,:66], data_X[...,66:66+15], data_X[...,-(9+4):-4], Train_data_root, b_first=True)
    else:
        gen_Scaler_From_Data_inFolder(f"{Train_data_root}/{i}",data_X[...,:66], data_X[...,66:66+15], data_X[...,-(3+4):-4], Train_data_root, b_first=True)

""" Generate Scaler for Each Features """
scaler = joblib.load(f'{Train_data_root}/npz/mixamo.pkl')
...
mean_ee = mean[66: 66 + 15]
scale_ee = vars[66: 66 + 15]

mean_vel = mean[66+15:]
scale_vel = vars[66+15:]


np.savetxt(f'{Train_data_root}/npz/mean_pos.txt',mean_pos,delimiter=" ")
np.savetxt(f'{Train_data_root}/npz/vars_pos.txt',scale_pos,delimiter=" ")

np.savetxt(f'{Train_data_root}/npz/mean_ee.txt',mean_ee,delimiter=" ")
np.savetxt(f'{Train_data_root}/npz/vars_ee.txt',scale_ee,delimiter=" ")

np.savetxt(f'{Train_data_root}/npz/mean_vel.txt',mean_vel,delimiter=" ")
np.savetxt(f'{Train_data_root}/npz/vars_vel.txt',scale_vel,delimiter=" ")

print("finished scaler update")


""" Generate Sequential Train Data (Network Input) """
folder_num = 1
start_num = 0
for i in range(0,folder_num):
    data_X = np.load(f"{Train_data_root}/train_{i}.npz")['clips'].astype(np.float32)

    joint_X = exputils.Normalize_data(data_X[...,:66],mean_pos,scale_pos)
    ee_X = exputils.Normalize_data(data_X[...,66:66+15],mean_ee,scale_ee)
    if(b_target_isMat):
        tar_root_X = exputils.Normalize_data(data_X[...,-(9+4):-4],mean_vel,scale_vel)
        start_num = save_split_trainable_data(10,
                                          joint_X,ee_X,tar_root_X,
                                          data_X[...,66+15:-(9+4)],data_X[...,-4:],
                                          start_num,True,f"{Train_data_root}/npz/train")
    else:
        tar_root_X = exputils.Normalize_data(data_X[...,-(3+4):-4],mean_vel,scale_vel)
        start_num = save_split_trainable_data(10,
                                          joint_X,ee_X,tar_root_X,
                                          data_X[...,66+15:-(3+4)],data_X[...,-4:],
                                          start_num,True,f"{Train_data_root}/npz/train")

""" Generate Sequential Test Data (Network Input) """
#test_data root
create_dir_save_trainable_data = f'{Test_data_root}/npz'
if not os.path.exists(create_dir_save_trainable_data):
    os.makedirs(create_dir_save_trainable_data)
...
start_num = 0
for i in range(0,folder_num):
    data_X = load_npz_data_inFolder(f"{Test_data_root}/{i}", f"{Test_data_root}/test")

    data_X = np.load(f"{Test_data_root}/test_{i}.npz")['clips'].astype(np.float32)
    ...
    if(b_target_isMat):
        ...
        start_num = save_split_trainable_data(10,
                                            joint_X,ee_X,tar_root_X,
                                            data_X[...,66+15:-(9+4)],data_X[...,-4:],
                                            start_num,True,f"{Test_data_root}/npz/test")
    else:
        tar_root_X = exputils.Normalize_data(data_X[...,-(3+4):-4],mean_vel,scale_vel)
        start_num = save_split_trainable_data(10,
                                            joint_X,ee_X,tar_root_X,
...
# scaler_save_root = "/root/home/project/Data/root_position_foot_all"

# #loco
# data_save_root = "/root/home/project/Data/sliced_root_position_foot/loco"
# for i in range(0,10):
#     if i == ind_loco:
#         gen_Train_Valid_Test_Data(f"{data_save_root}/sep{i}", scaler_save_root,b_split_data=True, b_first=True,b_pos=b_pos,b_pos_rot=b_pos_rot,b_ori_rot=b_ori_rot)
#     else:
#         gen_Train_Valid_Test_Data(f"{data_save_root}/sep{i}", scaler_save_root,b_split_data=False, b_first=False,b_pos=b_pos,b_pos_rot=b_pos_rot,b_ori_rot=b_ori_rot)
# #mani
# data_save_root = "/root/home/project/Data/sliced_root_position_foot/mani"
# for i in range(0,10):
#     if i == ind_mani:
#         gen_Train_Valid_Test_Data(f"{data_save_root}/sep{i}", scaler_save_root,b_split_data=True, b_first=False,b_pos=b_pos,b_pos_rot=b_pos_rot,b_ori_rot=b_ori_rot)
#     else:
#         gen_Train_Valid_Test_Data(f"{data_save_root}/sep{i}", scaler_save_root,b_split_data=False, b_first=False,b_pos=b_pos,b_pos_rot=b_pos_rot,b_ori_rot=b_ori_rot)
# #trans
# data_save_root = "/root/home/project/Data/sliced_root_position_foot/trans"
# for i in range(0,20):
#     if i == ind_trans:
#         gen_Train_Valid_Test_Data(f"{data_save_root}/sep{i}", scaler_save_root,b_split_data=True, b_first=False,b_pos=b_pos,b_pos_rot=b_pos_rot,b_ori_rot=b_ori_rot)
#     else:
...
#     mean_ee = mean[66+3: 66+3 + 30]
#     vars_ee = vars[66+3: 66+3 + 30]

#     mean_vel = mean[66+3+30:]
#     vars_vel = vars[66+3+30:]
