...
class TrainDataset_Rot(Dataset):
    """
    Motion dataset. 
    ...
    def __init__(self, dataroot, seqlen, dropout):
        ...
        self.indice = []
        # load file names
        self.fnamesTrainX, self.fnamesTrainCond, self.fnamesTrainDescriptor, self.fnamesTrainLabel, self.fnamesTrainEnv, self.fnamesTrainEE = self._make_dataset(dataroot)
        
        self.ee_dim = 5*3*2 # Head, LH, RH, RF, LF 순서로 가자
        self.ee_feature = 3
        
        # for drop condition
        self.zeromask_ee= np.zeros((self.ee_dim,4),dtype=np.float32)
        self.zeromask_ee[:3*self.ee_feature*2,0] = True # upper end effector
        self.zeromask_ee[-2*self.ee_feature*2:,1] = True # lower end effector 
    ...
    def _make_dataset(self, root):
        ...
        return fnamesTrainX, fnamesTrainCond, fnamesTrainDescriptor, fnamesTrainLabel, fnamesTrainEnv, fnamesTrainEE
    ...
    def __getitem__(self, idx):
        ...
        #202207
        self.ee = np.load(self.fnamesTrainEE[idx])['clips'].astype(np.float32)
        ...
        self.ee = np.swapaxes((self.ee), 0, 1)
        ...
        if self.dropout>0.:
            ...
            # 20220419. descriptor 얻기 위해서 해당 condition 을 가져온다.
            sample = {'x': self.x[:,:], 'cond': cond_masked,'ee_gt': self.ee, 'ee_cond' :ee_cond_masked, 'descriptor' : des_masked, 'label':self.label[:,:]}
        else:
            sample = {'x': self.x[:,:], 'cond': self.cond[:,:],'ee_cond':self.ee_cond,'descriptor' : self.descriptor[:,:],'label':self.label[:,:]}
            
        return sample
class ValidDataset_Rot(Dataset):
    """
    Motion dataset. 
    Prepares conditioning information (previous poses + control signal) and the corresponding next poses"""
    ...
    def __init__(self, dataroot, seqlen, dropout):
        ...
        self.fnamesValidX, self.fnamesValidCond, self.fnamesValidDescriptor, self.fnamesValidLabel, self.fnamesValidEnv, self.fnamesValidEE = self._make_dataset(dataroot)
    ...
    def _make_dataset(self, root):
        ...
        #
        fnamesValidEE = []
        for filename in sorted(glob.iglob(root + '/valid_scaled_ee_*.npz', recursive=True)):
            fnamesValidEE.append(filename)
        
        
        return fnamesValidX, fnamesValidCond, fnamesValidDescriptor, fnamesValidLabel, fnamesValidEnv, fnamesValidEE                                                                     
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        # ee
        self.ee = np.load(self.fnamesValidEE[idx])['clips'].astype(np.float32)
