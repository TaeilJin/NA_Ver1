...
class TrainDataset_Inpainting_TARGET(Dataset):
    """
    Motion dataset. 
    ...
    def _make_dataset(self, root):
        ...
        # 20221201. foot 을 얻었다.
        fnamesTrainRoot = []
        for filename in sorted(glob.iglob(root + '/train_scaled_root_estimate_input_*.npz', recursive=True)):
            fnamesTrainRoot.append(filename) 

        return fnamesTrainX, fnamesTrainCond, fnamesTrainDescriptor, fnamesTrainLabel, fnamesTrainEnv,fnamesTrainFoot,fnamesTrainRoot
    ...
    def __getitem__(self, idx):
        ...
        if self.dropout is not None:
            ...
            # end-effector 를 batch 마다 random 하게 줘봤다
            # ee_cond_p0 = np.repeat(self.zeromask_ee[:,self.zeroEEcondSeq[0]:self.zeroEEcondSeq[0]+1],div,axis=-1)
            # ee_cond_p1 = np.repeat(self.zeromask_ee[:,self.zeroEEcondSeq[0]:self.zeroEEcondSeq[0]+1],div,axis=-1)
            # ee_cond_p2 = np.repeat(self.zeromask_ee[:,self.zeroEEcondSeq[0]:self.zeroEEcondSeq[0]+1],div,axis=-1)
            # ee_cond_p3 = np.repeat(self.zeromask_ee[:,self.zeroEEcondSeq[0]:self.zeroEEcondSeq[0]+1],tt-div*3,axis=-1)
