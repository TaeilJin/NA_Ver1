...
class TrainDataset_Inpainting_five(Dataset):
    """
    Motion dataset. 
    ...
    def __init__(self, dataroot, seqlen, dropout):
        ...
        self.ee_dim = 3*3 # Head, LH, RH, RF, LF 순서로 가자
        self.ee_feature = 3
        # for drop condition
        self.zeromask_ee= np.zeros((self.ee_dim,3),dtype=np.float32)
        self.zeromask_ee[:3,0] = True # upper end effector
        #self.zeromask_ee[-6:,1] = True # lower end effector 
        self.zeromask_ee[3:9,1] = True # all end effector
        self.zeromask_ee[:,2] = False # all end effector is zero

        self.zeroEEcondSeq = [0,1,2] # All, upper only, lower only, zero
    ...
    def __getitem__(self, idx):
        ...
        if self.dropout>0.:
            ...
            ee_cond_p2 = np.repeat(self.zeromask_ee[:,self.zeroEEcondSeq[2]:self.zeroEEcondSeq[2]+1],tt-div*2,axis=-1)
            
            ee_cond_mask = np.concatenate((ee_cond_p0,ee_cond_p1,ee_cond_p2), axis = -1)
        ...
        return sample
class ValidDataset_Inpainting_five(Dataset):
    """
    Motion dataset. 
    Prepares conditioning information (previous poses + control signal) and the corresponding next poses"""
    ...
    def __getitem__(self, idx):
        ...
        return sample
class TestDataset_Inpainting_five(Dataset):
    """Test dataset."""
