...
from nflows.transforms.base import Transform,SplittedTransform
...
class SplitedInputsAwareCouplingTransform(SplittedTransform):
    """A base class for Coupling Transform steps,
    """
    def __init__(self, in_channels, unconditional_transform=None):
        """
        Constructor.
        ...
        self.num_identity_features = in_channels // 2
    ...
    def forward(self, identity_split, transform_split,transform_params, context=None):
        
        logabsdet = 0.0
        # transform the transform
        #transform_params = self.transform_net(identity_split, context)
        transform_split_o, logabsdet = self._coupling_transform_forward(
        ...
        if self.unconditional_transform is not None:
            identity_split_o, logabsdet_identity = self.unconditional_transform(
                identity_split, context
            )
            logabsdet += logabsdet_identity
        else :
            identity_split_o = identity_split

        return identity_split_o,transform_split_o, logabsdet

    def inverse(self, identity_split,transform_split,transform_params, context=None):
       
        logabsdet = 0.0
        if self.unconditional_transform is not None:
            identity_split_o, logabsdet = self.unconditional_transform.inverse(
                identity_split, context
            )
        else :
            identity_split_o = identity_split
        ...
        #transform_params = self.transform_net(identity_split, context)
        transform_split_o, logabsdet_split = self._coupling_transform_inverse(
            inputs=transform_split, transform_params=transform_params
        )
...
class SplitedInputsAwarePiecewiseCouplingTransform(SplitedInputsAwareCouplingTransform):
    def _coupling_transform_forward(self, inputs, transform_params):
...
class SplitedInputsAwarePiecewiseRationalQuadraticCouplingTransform(SplitedInputsAwarePiecewiseCouplingTransform):
    def __init__(
        self,
        in_channels,
        hidden_channels,
    ...
    def _piecewise_cdf(self, inputs, transform_params, inverse=False):
        ...
        if self.hidden_channels is not None:
            unnormalized_widths /= np.sqrt(self.hidden_channels)
            unnormalized_heights /= np.sqrt(self.hidden_channels)
        else:
            warnings.warn(
                "Inputs to the softmax are not scaled down: initialization might be bad."
            )
